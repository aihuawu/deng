...
address 0xa9876 {
	...
	module amoeba_employee_payroll_scheduler {
		// use Std::Signer;
		// use 0xa9876::amoeba_map;
		use 0xa9876::amoeba_timestamp;
		use 0xa9876::amoeba_employee_payroll;
		// use 0xa9876::amoeba_next_id;
		// use 0xa9876::amoeba_safe_math_util;

		struct Ty_payroll_next has copy, drop, store { 
		...
		public fun check_next(n: &mut Ty_payroll_next)
		{
			let ts = amoeba_timestamp::current_time_ms();
			if(ts > n.last_event_time + n.interval){
				amoeba_employee_payroll::create_item();
